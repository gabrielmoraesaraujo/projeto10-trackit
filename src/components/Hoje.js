...
function HabitoHoje({habito, index, marcaConcluido, desmarcarConcluido}){
    ...
    return(
        <HabitoContainer>
            <Esquerda>
                <h2>{habito.name}</h2>
                <div>
                    <SequenciaAtual cor={habito.currentSequence === habito.highestSequence? "green" : {color}}>Sequencia atual: <span>{habito.currentSequence} dias</span></SequenciaAtual>
                    <SeuRecorde cor={habito.currentSequence === habito.highesSequence? "green" : "#666666"}>Seu recorde: <span>{habito.highesSequence} dias</span> </SeuRecorde>
                </div>
            </Esquerda>
            <Direita>
                <Concluindo on click={mandaMarcarConcluido} cor={habito.done ? "#8FC549" : "#E7E7E7"}>
...
function PorcentagemHabitos({porcentagem}){
    ...
    if(percent > 0){
        return(
            <p style={{color: "#8FC549"}}>{percent}% dos habitos concluidos hoje</p>
        );
    }else{
        return(
            <p>Nenhum hábito concluido hoje</p>
        );
    }
}
